...
private:
  void reverse_destroy_n(iterator begin, std::size_t n) noexcept {
    std::destroy(std::reverse_iterator<iterator>(begin + n), std::reverse_iterator<iterator>(begin));
  }

  void uninitialized_copy_n_with_reverse_destroy(iterator dest, iterator from, std::size_t n) {
    iterator cur = dest;
  ...
  pointer reallocate(std::size_t new_capacity) {
    auto new_data = static_cast<pointer>(operator new(new_capacity * sizeof(value_type)));
    try {
  ...
  iterator no_const(const_iterator it) {
...
public:
  ...
  vector(const vector& other)
      : size_(other.size_)
      , capacity_(size_)
      , data_(other.capacity_ ? static_cast<pointer>(operator new(other.size_ * sizeof(value_type))) : nullptr) {
    try {
      uninitialized_copy_n_with_reverse_destroy(data_, other.data_, other.size_);
    } catch (...) {
      operator delete(data_);
      throw;
    }
  ...
  iterator erase(const_iterator first, const_iterator last) {
    iterator ret_it = no_const(first), last_ = no_const(last), first_ = no_const(first);
    while (last_ != end()) {
